...
class ValuedBasedPolicy(Policy):
    ...
    def sample(self, state: State) -> Action:
        ...
        """
        action_values = {
            action: self.strategy.estimate_q(state, action) for action in Action
        }
        ...
        probabilities = {
            action: (
                epsilon / m + (1 - epsilon) if action == best_action else (epsilon / m)
            )
...
class ControlStrategy:
    ...
    def get_plot_df(self) -> pd.DataFrame:
        ...
        max_q = {state: max(self.q[state].values()) for state in self.q.keys()}
...
class LinearFunctionApproximationSarsaLambdaControlStrategy(ControlStrategy):
    ...
    def _policy_iteration(self) -> None:
        ...
        delta = (
            reward
            + (
                self.estimate_q(new_state, new_action)
                if not new_state.is_terminal
                else 0
            )
